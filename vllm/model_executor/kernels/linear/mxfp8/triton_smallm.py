...
Block scale interpretation:
    ...
    so output[m, n] = sum over K of (input[m, k] * weight[n, k])
                    = sum over k_block of (
                          sum_{i=0..31} input_fp8[m, k_block*32+i] * weight_fp8[n, k_block*32+i]
                      ) * 2^(input_scale[m, k_block] + weight_scale[n, k_block] - 254)
"""
...
def mxfp8_smallm_gemm(
    ...
    # Ensure contiguous; both inputs and scales should normally be contiguous
    # but defensively re-stride if not.
    input_fp8_c = input_fp8 if input_fp8.is_contiguous() else input_fp8.contiguous()
    weight_fp8_c = weight_fp8 if weight_fp8.is_contiguous() else weight_fp8.contiguous()
    input_scale_c = input_scale if input_scale.is_contiguous() else input_scale.contiguous()
    weight_scale_c = weight_scale if weight_scale.is_contiguous() else weight_scale.contiguous()
